# Mac Setting
- ~~`言語と地域` から英語を最優先言語にしておく~~
- できるだけdefaults.shで設定
- 都度書き換える

---

## システム環境設定

### Apple ID
- サインイン
- `iCloud`: `写真` `メール` `株価`: オフ

### キーボード
- `キーボードナビゲーション`: オン
- `ショートカット`
    - `修飾キー` で `Caps Lock` を `Ctrl` に変更
    - `キーボード` > `次のウィンドウを操作対象にする`: `⌥tab`
    - `Spotlight` > 全て無効化
    - `スクリーンショット` > `⌘⇧5` 以外無効化
- `入力ソース`
    1. 左下の`+`から日本語→`ひらがな(Google)`、英語→`英数(Google)`を追加
    2. もともとあったMacの`日本語`を選択し、`入力モード`の`英字`にチェック
    3. これでもとのMacの`US``日本語`を消せる

### 一般 > 言語と地域
- システム言語を英語にした場合は、第2言語に日本語を追加しておく
- `詳細設定` > `週の最初の曜日`: `月曜日`

### Apple IntelligenceとSiri
- Apple Intelligence、Siriそれぞれオンにする

### Spotlight
- 使わないからいろいろオフでOK. ただし`アプリ`だけはON
- [MarLionとの競合](https://u-next.slack.com/archives/C04NBU5TCE7/p1693558194119799?thread_ts=1693274396.529599&cid=C04NBU5TCE7)
- `検索のプライバシー` > `projects` フォルダを追加(重くなるので)

### アクセシビリティ
- `ズーム` > `スクロールジェスチャと修飾キーを使って拡大縮小` オン
- `ポインタコントロール`
    - `スプリングローディング`を短くしておく
    - `トラックパッドオプション` > `ドラッグにトラックパッドを使用`（ダブルタップでドラッグできるようになる）
- `ディスプレイ` > `カーソル` > `カーソルサイズ`: 10%

### ディスプレイ
- `Night Shift` > `スケジュール`: `日の入りから日の出まで` `色温度`: 最大

### デスクトップとDock
- `最近使用したアプリをステージマネージャに表示` オフ
- `デスクトップ上のウィジェットを淡色表示`: `常に`
- `デフォルトのWebブラウザ`: `Google Chrome`
- `Mission Control` > `最近の使用状況に、、、`: オフ

#### Mission Control
- Dockの拡大を３０％くらいに
- `ウィンドウをアプリケーションごとにグループ化`: オン
- `ホットコーナー` > 右下を`スクリーンセーバー`、左下を`アプリ`

### メニューバー
- `最近使った書類、アプリケーション、およびサーバの表示数`: なし
- `時計` > `時刻内の":"を点滅`: オン
- `Spotlight`: オフ
- `Bluetooth`: 表示
- `Sound`: 常に表示

### 外観
- `外観モード`: `自動`
- `Liquid Glass`: `色合い調整`
- `カラー`: `グラファイト`
- `スクロールバーをクリックして`: `クリックされた部分へジャンプ`

### 壁紙
- デスクトップ画像を変更
- `スクリーンセーバーを開始`: `しない`

### 通知
- `カレンダー` `メール` `ヒント`: オフ
- `Chrome` 等: オン（各アプリ起動時に設定すれば良い）
- `画面がロックされているとき`: オフ
- `iPhoneからの通知を許可`: オフ

### サウンド
- `起動時にサウンドを再生`: オフ

### 集中モード
- `デバイス間で共有`: オフ
- `おやすみモード`: 20~6時

### ロック画面
適宜設定

<!-- ### プライバシーとセキュリティ
- `ファイアウォール` > `ファイアウォールをオンにする` -->

### Touch IDとパスワード
- 登録

### インターネットアカウント
- Gmail*2を追加 > `カレンダー`と`連絡先`だけ?: オン

### トラックパッド
- `クリック`: `弱い`
- `軌跡の速さ`: 4
- `その他のジェスチャ`
    - `フルスクリーンアプリケーション間をスワイプ`: `４本指`
    - `App Exposé`: `４本指で下にスワイプ`


---


## Finder

### 環境設定
- `サイドバー`: 以下のみチェック
  - `アプリケーション`
  - `デスクトップ`
  - `ダウンロード`
  - `a-hirata`
  - `外部ディスク`
  - `CD・DVD`
  - `Bonjourコンピュータ`
  - `AirDrop`
  - `接続中のサーバ`

### デスクトップ
デスクトップを右クリック > `表示オプションを表示`
- `並べ替え`: `種類`
- `表示順序`: `グリッドに沿う`
- `項目の情報を表示`: チェック


---


## 外観設定
- デスクトップを５個に増やす
- Dockでアプリを右クリック → `オプション` → デスクトップを指定しておく
    - Finder: `すべてのデスクトップ`
    - Chrome: `デスクトップ3`
    - Figmaなどデザインツール: `デスクトップ2`
    - Slack, LINE, Discordなどコミュニケーションツール: `デスクトップ4`
- Finderツールバーを右クリックしてカスタマイズ
    <img width="738" height="50" alt="image" src="https://github.com/user-attachments/assets/8ad1c380-acb9-4b11-aab5-e699461fc8dd" />
    - 不要なものを消す
    - アプリケーションフォルダから`Chrome` `Code` `Ghostty` `Ps` `Ai` を `⌘ドラッグ` で追加
    - `AirDrop` を追加
- サイドバーに追加
    - `projects`
    - クラウドストレージ
- 外部接続時のデスクトップと通知センターに `カレンダー` `天気` `Craft` のウィジェットを設定。天気は右クリックで現在地にしておく
- Dock整理
    <img width="914" height="144" alt="image" src="https://github.com/user-attachments/assets/5b6ff714-b640-432c-b1bc-c2d44ab959fe" />
